...
export class ScheduleAiFlowService {
  ...
  async heuristicDraft(params: GenerateScheduleDto) {
    ...
    // Карта выбранных групп для быстрого поиска
    const selectedGroupIds = new Set(params.groupIds);
    ...
    this.logger.log(`Loaded ${studyPlans.length} studyPlans, filtering for groups: ${params.groupIds.join(', ')}, subjectIds: ${params.subjectIds?.join(', ') || 'none'}`);

    // Логируем связи studyPlans с группами
    for (const sp of studyPlans) {
      const groupsInfo = (sp as any).group?.map((g: any) => `${g.id}(${g.name})`).join(', ') || 'no groups';
      this.logger.log(`StudyPlan ${sp.id} (${sp.name}) has groups: ${groupsInfo}`);
    }

    const selected = studyPlans.filter((sp: any) => {
      // Проверяем, связан ли studyPlan с выбранными группами
      if (sp.group && Array.isArray(sp.group)) {
        const hasMatchingGroup = sp.group.some((g: any) => params.groupIds.includes(g.id));
        if (hasMatchingGroup) {
          this.logger.log(`StudyPlan ${sp.id} (${sp.name}) selected - has matching groups`);
          return true;
        }
      }

      // Если явно указаны subjectIds, включаем даже без групп (для ручного выбора)
      if (params.subjectIds?.includes(sp.id)) {
        this.logger.log(`StudyPlan ${sp.id} (${sp.name}) selected - explicitly in subjectIds`);
        return true;
      }

      this.logger.log(`StudyPlan ${sp.id} (${sp.name}) filtered out - no matching groups`);
      return false;
    });

    this.logger.log(`After filtering: ${selected.length} studyPlans selected`);
    ...
    const pickClassroom = (subject: string, groupSize: number, date: string, start: string, end: string) => {
      ...
      if (/ИНФОРМ|ПРОГРАМ|COMPUT|IT/.test(subj)) preferred.push('COMPUTER');
      if (/ФИЗИК|ХИМИ|LAB|ЛАБ/.test(subj)) preferred.push('LABORATORY');
      if (/ЛЕКЦ|THEOR|ТЕОР/.test(subj)) preferred.push('LECTURE');
      if (/СЕМИН|ДИСК/.test(subj)) preferred.push('PRACTICE');
      if (/СПОРТ|ФИЗКУЛ|PE|GYM/.test(subj)) preferred.push('PRACTICE');
      if (/ПРАКТИК|WORK|МАСТЕР/.test(subj)) preferred.push('PRACTICE');
      if (preferred.length === 0) preferred.push('PRACTICE');
      return classrooms
    ...
    for (const sp of selected) {
      ...
      let targetGroups =
        params.groupIds?.length
          ? (sp as any).group.filter(g => params.groupIds.includes(g.id))
          : (sp as any).group; // ← fallback
      if (!targetGroups.length) {
        // Если studyPlan выбран явно (в subjectIds), но не связан с выбранными группами,
        // все равно размещаем для выбранных групп
        if (params.subjectIds?.includes(sp.id)) {
          targetGroups = groups.filter(g => params.groupIds.includes(g.id));
        } else {
          continue;
        }
      }
      ...
      for (const grp of targetGroups) {
        ...
        // Используем глобальный счетчик для равномерного распределения начальных слотов
        const baseSlotCursor = globalDayPointer % timeSlots.length;
        globalDayPointer = (globalDayPointer + 1) % timeSlots.length;
        ...
        while (cursor <= end && placed < totalLessons) {
          ...
          if (this.isWorkingDay(cursor, params.constraints.excludeWeekends)) {
            ...
            for (let idx = 0; idx < rotatedSlots.length; idx++) {
              ...
              const teacherPrefs = params.teacherPreferences?.[sp.teacherId];
              if (this.violatesTeacherPreferences(teacherPrefs, cursor, slotStart, slotEnd, dowNormalized)) continue;
              ...
              draft.push({
                ...
                teacherName: `${(sp as any).teacher.user.name} ${(sp as any).teacher.user.surname}`,
                groupId,
                groupName: grp.name,
                subject: sp.name,
                date: dateStr,
            ...
            // Увеличиваем slotCursor для разнообразия временных слотов в следующие дни (больше инкремент для лучшего распределения)
            slotCursor = (slotCursor + 2) % timeSlots.length;
          }
          cursor.setDate(cursor.getDate() + 1);
        }

        // Второй проход для неразмещенных уроков: ослабляем ограничения
          ...
          while (cursor <= end && placed < totalLessons) {
            ...
            if (this.isWorkingDay(cursor, params.constraints.excludeWeekends)) {
              ...
              for (let idx = 0; idx < rotatedSlots.length; idx++) {
                const slot = rotatedSlots[idx];
                if (placed >= totalLessons) break;
                if (placedTodayCount >= perDayQuota * 2) break; // увеличить квоту
                const slotStart = slot.start;
                const slotEnd = slot.end;
                // Игнорируем конфликты учителя и группы для черновика
                const groupId = grp.id;
                const currentWeekIndex = Math.floor((cursor.getTime() - start.getTime()) / (7 * 24 * 3600 * 1000));
                if ((perWeekPlaced.get(currentWeekIndex) || 0) >= weeklyLessons) continue;
                if (this.countGroupDayLessons(draft, groupId, dateStr) >= maxLessonsPerDay * 2) continue; // увеличить лимит
                // Игнорируем preferredDays, noFirst, noLast, lunchBreak, wouldExceedConsecutive
                const groupSize = groupSizeMap.get(groupId) || 0;
                const classroom = pickClassroom(sp.name, groupSize, dateStr, slotStart, slotEnd);
                // Игнорируем конфликт комнаты
                draft.push({
                  tempId: `${sp.id}-${groupId}-${placed}-retry`,
                  studyPlanId: sp.id,
                  teacherId: sp.teacherId,
                  teacherName: `${(sp as any).teacher.user.name} ${(sp as any).teacher.user.surname}`,
                  groupId,
              ...
              // Увеличиваем slotCursor для разнообразия
              slotCursor = (slotCursor + 2) % timeSlots.length;
            }
            cursor.setDate(cursor.getDate() + 1);
          }
    ...
    try {
      ...
      for (const sp of studyPlans) {
        ...
        for (const g of (sp as any).group) {
  ...
  async optimizeLocal(draft: any[], params: GenerateScheduleDto) {
    ...
    const mapped: OptimizerParams = {
      ...
      forbiddenFirstSubjects: (params as any)?.constraints?.forbiddenFirstSubjects || [],
      forbiddenLastSubjects: (params as any)?.constraints?.forbiddenLastSubjects || [],
  ...
  private violatesTeacherPreferences(preferences: any, date: Date, startTime: string, endTime: string, dow: number): boolean {
    if (!preferences) return false;

    const dateStr = this.toLocalDateStr(date);

    // Check unavailable times
    if (preferences.unavailableTimes) {
      const unavailable = Array.isArray(preferences.unavailableTimes) ? preferences.unavailableTimes : [];
      for (const u of unavailable) {
        if (u.date === dateStr && this.intervalOverlap(u.startTime, u.endTime, startTime, endTime)) {
          return true;
        }
      }
    }

    // Check unacceptable days
    if (preferences.unacceptableDays && preferences.unacceptableDays.includes(dow)) {
      return true;
    }

    // Check max hours per day (rough check, not exact)
    if (preferences.maxHoursPerDay) {
      // This would require counting existing lessons, but for simplicity, skip for now
      // In a full implementation, we'd count teacher's lessons on this date
    }

    // Preferred times/days are not violations, just preferences
    return false;
  }
}
