...
import { CreateSystemUserDto, UpdateSystemUserDto, UserFilterDto, UserStatus } from './dto/user.dto';
...
export class SystemService {
  ...
  async getUsers(filter?: UserFilterDto) {
    ...
    if (filter?.status === UserStatus.INACTIVE) {
      (where).deletedAt = { not: null };
    } else if (filter?.status === UserStatus.ACTIVE) {
  ...
  async searchStudents(search: string) {
    ...
    return students.map((s) => ({
      id: s.id,
      name: `${s.user.name} ${s.user.surname}`.trim(),
      email: s.user.email,
      group: s.group?.name || null,
    }));
  }

  async setParentChildren(userIdStr: string, childrenIds: number[]) {
    ...
    await this.prisma.parent.update({
      where: { userId },
      data: { students: { set: (childrenIds || []).map((id) => ({ id: Number(id) })) } },
    });
    return { ok: true };
  }
  ...
  async createUser(data: CreateSystemUserDto) {
    ...
    // If created user is a teacher, create Teacher profile
    if (role === 'TEACHER') {
      try {
        ...
        // Если не удалось создать профиль преподавателя, удаляем пользователя
        await this.prisma.user.delete({ where: { id: user.id } });
        throw new BadRequestException('Не удалось создать профиль преподавателя');
      }
    }
  ...
  async updateUser(id: string, data: UpdateSystemUserDto) {
    ...
    if (data.status) {
      updateData.deletedAt = data.status === UserStatus.INACTIVE ? new Date() : null;
    }
      ...
      if (role === 'TEACHER') {
        try {
          await this.prisma.teacher.upsert({
            where: { userId: userId },
            update: {},
            create: { userId: userId, employmentType: 'STAFF' },
          });
        } catch {
          throw new BadRequestException('Не удалось создать профиль преподавателя');
        }
