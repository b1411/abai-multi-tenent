...
const HomeworkPage: React.FC = () => {
  ...
  const loadHomeworks = async () => {
    try {
      ...
      const response = await homeworkService.getHomeworks(filters);
  ...
  const loadStats = async () => {
    try {
      const statsData = await homeworkService.getHomeworkStats({
        lessonId: filters.lessonId,
        studentId: hasRole('STUDENT') ? user?.id : undefined,
