...
const HomeworkSubmissionsPage: React.FC = () => {
  ...
  const handleSaveGrade = async () => {
    ...
    try {
      ...
      // Пока используем заглушку для метода
      console.log('Оценивание:', {
  ...
  const getSubmissionStatus = (submission: HomeworkSubmission): 'submitted' | 'graded' => {
    return submission.lessonResult?.homeworkScore !== undefined ? 'graded' : 'submitted';
  };
                  ...
                  <div className="flex items-center space-x-2 ml-4">
                    {submission.fileUrl && (
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => window.open(submission.fileUrl?.url, '_blank')}
                      >
